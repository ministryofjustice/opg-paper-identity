...
class IIQService
{
    ...
    public function startAuthenticationAttempt(array $saaRequest): array
    {
        return $this->withAuthentication(function () use ($saaRequest) {
            ...
            if ($request->SAAResult->Results->Outcome === 'Insufficient Questions (Unable to Authenticate)' && $request->SAAResult->Results->NextTransId->string === 'END') {
                return ['questions' => [], 'control' => $control];
            }

            if ($request->SAAResult->Results->Outcome === 'Authentication Questions returned' && $request->SAAResult->Results->NextTransId->string === 'RTQ') {
