...
disable_scanner_rules() {
    ...
    curl --fail "localhost:8080/JSON/ascan/action/disableScanners?ids=20015,40018,40019,40020,40021,40022,40024,40027,40043,40045,90034,40042"
}
...
active_scan(){
    ...
    curl --fail "localhost:8080/JSON/ascan/action/setOptionMaxAlertsPerRule?Integer=10"

    # Config Zap's number of parallel active scanners
    curl --fail "localhost:8080/JSON/ascan/action/setOptionHostPerScan?Integer=5"
    ...
    # Start Active Scan
    curl --fail "localhost:8080/JSON/ascan/action/scan?url=$SCAN_URL"
    ...
    do
        STATUS=$(curl --fail --silent "localhost:8080/JSON/ascan/view/scans" | jq '.scans[] | select(.id == "0").state')
        CURRENT_RUN_TIME=$SECONDS
    ...
    then
        ...
        curl --fail "localhost:8080/JSON/ascan/action/stopAllScans/"
...
filter_false_positives() {
    ...
    do
        ...
        curl --fail "localhost:8080/JSON/alertFilter/action/addGlobalAlertFilter?ruleId="$ALERT"&newLevel=-1&enabled=true"
    done
    # Apply All Global Alert Filters
    echo "INFO - Applying all enabled Global Results Filters"
    curl --fail "localhost:8080/JSON/alertFilter/action/applyGlobal"
}
